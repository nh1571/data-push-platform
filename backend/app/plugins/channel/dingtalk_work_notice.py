...
async work-notice (markdown) via ``topapi/message/corpconversation/asyncsend_v2``.
"""
...
class DingTalkWorkNoticePlugin:
    ...
    def _build_msg_body(self, config: dict[str, Any], message: Message) -> dict[str, Any]:
    ...
    def send(self, config: dict[str, Any], message: Message) -> DeliveryResult:
        """Obtain access token then async-send a markdown work notice."""
        try:
        ...
        try:
            with httpx.Client(timeout=_DEFAULT_TIMEOUT) as client:
                ...
                payload = self._build_msg_body(config, message)
                resp = client.post(
                    _SEND_URL,
