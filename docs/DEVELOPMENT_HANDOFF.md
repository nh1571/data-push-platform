...
### 0.0 核心产品模型：模板 ≠ 静态成品（用户确认 · 2026-07-17）

```text
┌─────────────────────────────────────────────────────────────┐
│  内容工作台（编辑态）= 制作「推送模板」                        │
│  · SQL + {{参数}} 定义（yesterday 等可自动解析）              │
│  · 组件绑定字段（不写死业务数值）                              │
│  · 画布版式 + 图外文案壳（可含 {{字段}}）                      │
│  · 保存进 PushJob.render_spec（artboard v3），不存最终 PNG   │
└───────────────────────────┬─────────────────────────────────┘
                            │ 每次 JobRun / 试推 / 调度
                            ▼
┌─────────────────────────────────────────────────────────────┐
│  运行态 = 按预置参数动态渲染                                   │
│  1. resolve SQL 参数（当日/昨日… + 运行时 overrides）         │
│  2. 数据源插件实时取数                                       │
│  3. artboard_to_message：组件树 + 文案壳 → 成图 + Markdown   │
│  4. 通道插件投递（钉钉 OpenAPI / Webhook 等）                 │
└─────────────────────────────────────────────────────────────┘
```

| 概念 | 是 | 不是 |
|------|----|------|
| 工作台产出 | **模板**（结构、绑定、版式、参数定义） | 某一天的静态报表截图存档 |
| 预览 | 用**当前样例/当前参数**预演终片 | 生产只发这一张缓存图 |
| 正式推送 | **每次重新取数 + 重新成图** | 复用上次 PNG |

代码落点：`execution/pipeline.py`（JobRun）与 `studio/service.py`（compile/test-push）共用 `artboard_to_message`；参数见 `sql_params.py`。
