...
> **分支怎么开：** 见 [COLLAB_GIT.md](./COLLAB_GIT.md)（分工≠永久分支）。  
> **Agent 技能：** `.grok/skills/data-push-collab/SKILL.md`  
> **产品笔记：** [product/](./product/)（原 Obsidian 已入库）
...
| 人 | 职责线 | 一句话 | 主要目录 | 功能分支前缀 |
|----|--------|--------|----------|--------------|
| **甲** | **内容工作台 + 成图** | 模板怎么配、预览/终片 | 见下「甲」 | `feature/studio-*` |
| **乙** | **通道 + 数据源** | 从哪取数、往哪发 | 见下「乙」 | `feature/io-*` |
| **丙** | **任务执行 + 调度 + 运营页** | 何时跑、跑得怎样 | 见下「丙」 | `feature/runtime-*` |

平台/部署/登录（`config`、`scripts`、`auth`）谁顺手谁改，大改群里说一声。

**分支原则（重要）：**  
- 集成线永远是 **`main`**。  
- 每人做**一个任务**时从 main 拉一条 **短** 功能分支，做完 PR 合回 main 再删。  
- **不要**维护「甲永久 branch-studio」这种长期分工分支（会合并爆炸）。
