...
> **默认 3 条线（studio / io / runtime）只是起步，不是上限。**  
> 加人、加领域：改 [**LANES.md**](./LANES.md) 注册表即可，不必重定 Git 规矩。  
> **分支：** [COLLAB_GIT.md](./COLLAB_GIT.md) · **技能：** `data-push-collab` · **产品：** [product/](./product/)
...
权威表见 **[LANES.md](./LANES.md)**。摘要：

| 线 | 一句话 | 分支前缀 |
|----|--------|----------|
| **studio** | 工作台 + 成图 | `feature/studio-*` |
| **io** | 数据源 + 通道 | `feature/io-*` |
| **runtime** | 执行 + 调度 + 运营页 | `feature/runtime-*` |

**人变多时：**

| 情况 | 怎么办 |
|------|--------|
| 同一线活多人 | **共线**：子目录认领，仍用同一 `feature/<lane>-*` 前缀 |
| 新领域稳定出现 | 在 **LANES.md 加一行**（新 laneId + 目录 + 标签） |
| 只是临时支援 | 不改表，PR 说明「支援 xx 线」 |

**分支原则：** `main` + **短**功能分支；不要「一人一条永久分支」。
...
```text
main
 ├── feature/studio-xxx      # 甲 · 单任务
 ├── feature/io-xxx          # 乙 · 单任务
 └── feature/runtime-xxx     # 丙 · 单任务
```

小步 PR → squash 合入 main → 删功能分支。主线保持 `./scripts/dev.sh` 可跑。
